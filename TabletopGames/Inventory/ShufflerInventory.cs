...
/// <summary>
/// Same as StackContainerInventory, but with shuffler-specific logic.
/// Supports items with 'StackSize = 1' only!
/// </summary>
public class ShufflerInventory : InventoryBase
{
    ...
    public ShufflerInventory(ICoreAPI api, int quantitySlots = 1) : this(inventoryID: "shuffler-" + stackContainerId++, api: api)
    {
        this.quantitySlots = quantitySlots;
        slots = GenEmptySlots(quantitySlots);
    }

    private ShufflerInventory(string inventoryID, ICoreAPI api)
        : base(inventoryID, api)
    {
    }

    private ShufflerInventory(string className, string instanceID, ICoreAPI api)
